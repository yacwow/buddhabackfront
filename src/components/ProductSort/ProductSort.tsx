...
import styles from './ProductSort.less';
import ProductSortHeader from './ProductSortHeader/ProductSortHeader';
...
const App: React.FC = () => {
  ...
  const refreshBodyData = (page: number) => {
    let path = location.pathname;
    let arr = path.split('/');
    console.log(arr);
    if (arr.length === 5) {
  ...
  const handleAddProductListData = (e: any) => {
    ...
    let newCategory = getQueryCategory();
    ...
    request(`/admin/getSingleProductInfo`, {
      params: {
  ...
  const handleSubmit = () => {
    ...
    if (arr.length >= 5) {
      ...
      console.log(list);
      request('/admin/secure/updateCategoryRank', {
        params: {
          categoryInfo: decodedString,
      ...
      message.error({ content: 'url有点问题，大概率是程序的问题' }, 4);
    }
  };
  //用来传递给header的
  ...
  const handleRefresh = () => {
    ...
    if (arr.length >= 5) {
      const decodedString = decodeURIComponent(arr[4]);
      request('/admin/secure/updateAllCategoryRank', {
        params: {
          categoryInfo: decodedString,
        },
      }).then((data) => {
  ...
  return (
    <div style={{ position: 'relative', width: 1200, margin: '0px auto' }}>
      <Modal
        title="当前页批量插入确认"
        open={isModalOpen}
